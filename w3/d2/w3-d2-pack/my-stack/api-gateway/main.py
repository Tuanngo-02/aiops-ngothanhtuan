...
DNS_RESOLVER_URL = os.getenv("DNS_RESOLVER_URL", "http://dns-resolver:8006")
CACHE_SVC_URL = os.getenv("CACHE_SVC_URL", "http://cache-svc:8007")

async def forward_request(service_name: str, base_url: str, request: Request):
    ...
    try:
        ...
        UPSTREAM_LATENCY.labels(service=service_name, endpoint=request.url.path).observe(time.time() - start_time)
        UPSTREAM_REQUEST_COUNT.labels(service=service_name, endpoint=request.url.path).inc()
...
async def forward_payment(path: str, request: Request):
    return await forward_request("payment-svc", PAYMENT_SVC_URL, request)
...
async def forward_inventory(path: str, request: Request):
    return await forward_request("inventory-svc", INVENTORY_SVC_URL, request)

@app.get("/notification/{path:path}")
async def forward_notification(path: str, request: Request):
    return await forward_request("notification-svc", NOTIFICATION_SVC_URL, request)

@app.get("/checkout/{path:path}")
async def forward_checkout(path: str, request: Request):
    return await forward_request("checkout-svc", CHECKOUT_SVC_URL, request)
...
async def forward_auth(path: str, request: Request):
    return await forward_request("auth-svc", AUTH_SVC_URL, request)

@app.get("/dns/{path:path}")
async def forward_dns(path: str, request: Request):
    return await forward_request("dns-resolver", DNS_RESOLVER_URL, request)

@app.get("/cache/{path:path}")
async def forward_cache(path: str, request: Request):
    return await forward_request("cache-svc", CACHE_SVC_URL, request)
