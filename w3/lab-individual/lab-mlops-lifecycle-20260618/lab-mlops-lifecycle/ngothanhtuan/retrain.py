...
def train_model_on_df(df: pd.DataFrame, contamination: float = 0.03, n_estimators: int = 100):
    """Huấn luyện IsolationForest bên trong Pipeline (chứa StandardScaler) trên DataFrame, trả về (pipeline_model, scaler, anomaly_rate, len(X))."""
    ...
    return pipeline_model, scaler, anomaly_rate, len(X)
...
def register_new_version(
    ...
    """Log pipeline model to MLflow, register as new version, tag 'staging'. Returns version string."""
    import pickle
    import tempfile
    ...
    with mlflow.start_run(run_name="retrain-triggered") as run:
        ...
        # Log scaler riêng biệt cho mục đích tương thích ngược
        ...
        # Đăng ký pipeline model hoàn chỉnh
...
def post_deploy_monitor(
    ...
    for cycle in range(1, cycles + 1):
        ...
        # model ở đây là Pipeline nên predict tự động chuẩn hóa dữ liệu
...
def main():
    ...
    pipeline_model, scaler, anomaly_rate, n_rows = train_model_on_df(
        combined_df,
    ...
    if args.holdout:
        ...
        if "anomaly_label" in holdout_df.columns:
            ...
            # Do không cần scale thủ công (đã có Pipeline), truyền trực tiếp raw features
            X_hold = holdout_df[FEATURES].dropna()
            y_true = holdout_df.loc[X_hold.index, "anomaly_label"].values
            raw = pipeline_model.predict(X_hold)
            y_pred = (raw == -1).astype(int)
            tp = int(((y_pred == 1) & (y_true == 1)).sum())
            fp = int(((y_pred == 1) & (y_true == 0)).sum())
            fn = int(((y_pred == 0) & (y_true == 1)).sum())
    ...
    new_version = register_new_version(
        pipeline_model, scaler, anomaly_rate, n_rows,
