...
case "$FAULT" in
  latency)
    DELAY="${PARAM:-200ms}"
    echo "[inject_fault] Adding ${DELAY} network latency to $CONTAINER via application layer workaround..."
    ...
    # Fix for Docker Desktop Windows: Write directly to /tmp/fault_latency.txt
    # This interacts with our modified service.py to dynamically change latency
    docker exec "$CONTAINER" sh -c "echo ${DELAY_MS} > /tmp/fault_latency.txt"
  ...
  clear-latency)
    echo "[inject_fault] Removing latency rules from $CONTAINER..."
    docker exec "$CONTAINER" rm -f /tmp/fault_latency.txt
